...
function Product({ product }: any) {
  ...
  return (
    ...
    <div className={cx(" flex flex-col  my-[100px] ",product.images.length > 1?"gap-[200px] w-[500px]":"gap-[30px] w-[700px]")}>
      
    <div className=" h-[500px] w-full bg-no-repeat bg-cover bg-center ">
        {product.images.length > 1 ?   <ImageSwitcher
          title={product.title}
          ...
          <div className="flex flex-row items-center">
            {product.size.map((siz: any, i: number) => {
              return (
                <button 
                onClick={() => setSize(siz)}
                key={i} className="flex flex-row items-center gap-2">
                  <div className={cx("w-8 h-8   rounded-full grid justify-center items-center mr-3",size===siz?"bg-green-200":"bg-gray-200")}>
                    {siz}
                  </div>
                </button>
              );
